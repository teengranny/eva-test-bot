...
# Актуальный контекст (данные Нетологии без YouTube)
FULL_CONTEXT = """
...
- 13 лет на рынке, резидент Сколково, официальная лицензия и диплом.
- Помощь в трудоустройстве (Карьерный центр) и налоговый вычет 13%.
...
def ask_eva(user_id: int, question: str) -> str:
    ...
    # ОБНОВЛЕННЫЙ ПРОМПТ (БЕЗ YOUTUBE)
    system_prompt = f"""
...
- В ответах про школу обязательно давай ссылку на сайт из контекста в формате кликабельного текста. Про YouTube не упоминай.
